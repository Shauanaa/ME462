...
class CommanderNode(Node):
    ...
    def cmd_callback0(self, msg):
        if len(msg.data) != 3:
            ...
            return
        send_str = f"a{self.constrain(msg.data[0]):+04}{self.constrain(msg.data[1]):+04}{self.constrain(msg.data[2], 0):+04}\n"
    ...
    def cmd_callback1(self, msg):
        if len(msg.data) != 3:
            self.get_logger().warning(f'Command should only have 3 elements, got {len(msg.data)}. Not sending the command.', throttle_duration_sec=1)
            return
        send_str = f"b{self.constrain(msg.data[0]):+04}{self.constrain(msg.data[1]):+04}{self.constrain(msg.data[2], 0):+04}\n"
        self.port.write(send_str.encode())
        print(send_str)
